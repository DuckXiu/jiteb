...
pub async fn register(
    ...
    .to_owned();
    // +++ 新增权限检查+count-1代码 +++
    let api_url = std::env::var("PERMISSION_API_URL")
        .unwrap_or_else(|_| "http://localhost:8080/api/decrement".to_string());

    let client = Client::new();
    let response = match client
        .post(&api_url)
        .header("Content-Type", "application/json")
        .json(&serde_json::json!({ "udid": udid }))
        .send()
        .await
    {
        Ok(res) => res,
        Err(e) => {
            log::error!("权限 API 请求失败: {:?}", e);
            return Err((StatusCode::INTERNAL_SERVER_ERROR, "无法验证权限"));
        }
    };

    if !response.status().is_success() {
        return Err((StatusCode::FORBIDDEN, "设备无访问权限"));
    }

    let response_body: Value = match response.json().await {
        Ok(body) => body,
        Err(e) => {
            log::error!("权限 API 响应解析失败: {:?}", e);
            return Err((StatusCode::INTERNAL_SERVER_ERROR, "权限验证失败"));
        }
    };

    if !response_body["allowed"]
        .as_bool()
        .unwrap_or(false)
    {
        return Err((StatusCode::FORBIDDEN, "设备未授权或次数用尽"));
    }
    // +++ 结束新增代码 +++
